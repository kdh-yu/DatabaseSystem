...
public class CalendarPanel implements ActionListener {
    ...
    // 화면디자인
	public CalendarPanel(ActionListener listener, String loginID, Connection connection) {
		...
		// 이벤트 로드
		String getEventQuery = "SELECT * FROM calendar.event WHERE userid = ? AND EXTRACT(DAY FROM start_date) = ?";
		try {
			PreparedStatement preparedStatement = this.connection.prepareStatement(getEventQuery);
			preparedStatement.setString(1, loginID);
			preparedStatement.setInt(2, month);
			evList = preparedStatement.executeQuery();
		} catch (Exception ex) {
	...
	public void day(int year, int month) {
		...
		for (int day = 1; day <= lastDay; day++) {
            ...
            String hasEvent = "ㅤ";
			String startDayEventQuery = "SELECT * FROM calendar.event WHERE EXTRACT(YEAR FROM start_date) = ? AND EXTRACT(MONTH FROM start_date) = ? AND EXTRACT(DAY FROM start_date) = ? AND userid = ?";
			try {
				PreparedStatement preparedStatement = this.connection.prepareStatement(startDayEventQuery);
				preparedStatement.setInt(1, year);
				preparedStatement.setInt(2, month);
				preparedStatement.setInt(3, day);
				preparedStatement.setString(4, this.loginID);
