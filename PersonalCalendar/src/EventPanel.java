...
public class EventPanel {
    ...
    ArrayList<JPanel> eventArrayList = new ArrayList<>();
    String loginID;
    

    public EventPanel(ActionListener listener, Connection connection, String userid) {
    ...
    public JScrollPane loadEvent(int selectedMonth, int selectedDay, ActionListener listener) {
        ...
        String getEventQuery = "SELECT * FROM calendar.event WHERE EXTRACT(MONTH FROM start_date) = ? AND EXTRACT(DAY FROM start_date) = ? AND userid = ?";
        try {
            ...
            while (rs.next()) {
                ...
                JLabel evntName = new JLabel("◼︎ "+rs.getString("name"));
                JButton evntButton = new JButton("⏍");
                evntButton.addActionListener(listener);
                evntButton.setActionCommand("ModifyEvent"+idx);
                ...
                event.add(evnt, gbc);
                eventArrayList.add(evnt);
                gbc.gridy++;
            }
        } catch (Exception ex) {
            System.out.println(ex.getMessage());
            int eventNum = 0;  // 데이터베이스에 연결, 이벤트 개수 불러와야 함
        }
        /* 
    ...
    */
    ...
    public JPanel getEventButtonPanel(int idx) {
        return eventArrayList.get(idx);
    }
}
